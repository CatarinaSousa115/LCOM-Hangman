...
int (read_controller) (uint8_t port, uint8_t *data, uint8_t mouse) {
    ...
    uint8_t attemps = 10;
    uint8_t status; //vai armazenar os status do keyboard ou do mouse
    ...
    while (attemps > 0) {
        ...
        if ((status & OUT_BUF_FULL) != 0) { //O Output buffer está full, logo existem dados para ler/ser processados
            ...
            if ((!mouse && is_mouse) != 0) return 1;  //estava à espera de output do teclado, mas recebeu do mouse
...
int (write_controller) (uint8_t port, uint8_t data) {
    ...
    uint8_t attemps = 10;
    uint8_t status; //vai armazenar os status do keyboard
    ...
    while (attemps > 0) {
        ...
        if ((status & IN_BUFF_FULL) == 0) { 


            //se existir algo a escrever (verificado pela mascara de bits aplicada no "status")
